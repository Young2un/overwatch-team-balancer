...
export default function ManageClient({ initialPlayers }: ManageClientProps) {
  ...
  const toggleRole = (role: Role, isEdit: boolean) => {
    const target = isEdit ? editingPlayer : newPlayer;
    const setTarget = isEdit ? setEditingPlayer : setNewPlayer;
    ...
    setTarget({ ...target, roles: newRoles, skills: target.skills } as any);
  };
  ...
  const togglePrimary = (role: Role, isEdit: boolean) => {
    ...
    const currentPrimary = target.primary || [];
    const newPrimary = currentPrimary.includes(role)
      ? currentPrimary.filter(r => r !== role)
      : [...currentPrimary, role];

    setTarget({ ...target, primary: newPrimary } as any);
  };
  ...
  const updateSkill = (role: Role, value: string, isEdit: boolean) => {
    ...
    const newSkills = { ...target.skills, [role]: numValue };
    
    setTarget({ ...target, skills: newSkills } as any);
  };
  ...
  const togglePreferredHero = (hero: string, isEdit: boolean) => {
    ...
    const currentHeroes = target.preferredHeroes || [];
    const newHeroes = currentHeroes.includes(hero)
      ? currentHeroes.filter(h => h !== hero)
      : [...currentHeroes, hero];
    
    setTarget({ ...target, preferredHeroes: newHeroes } as any);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-900">플레이어 관리 ({players.length}명)</h2>
        <button
      ...
      {isAdding && (
        <div className="bg-white border border-blue-200 rounded-lg p-6 shadow-sm">
          <h3 className="text-lg font-medium text-gray-900 mb-4">새 선수 추가</h3>
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">가능 포지션</label>
            ...
            {newPlayer.roles?.length ? (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">주 포지션</label>
                <div className="flex gap-4">
            ...
            {newPlayer.roles?.length ? (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">티어 선택 (SR 자동 입력)</label>
                  ...
                  {newPlayer.roles.map(role => (
                    <div key={role}>
                      ...
                      <select
                        ...
                        onChange={(e) => updateSkill(role, e.target.value, false)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {players.map(player => (
          <div key={player.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition">
            {editingPlayer?.id === player.id ? (
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="text-xs font-semibold text-gray-500">포지션</div>
                ...
                <div className="space-y-2">
                  <div className="text-xs font-semibold text-gray-500">티어</div>
                  <div className="grid grid-cols-1 gap-2">
                    {editingPlayer.roles.map(role => (
                      <div key={role} className="flex items-center gap-2">
                         <span className="text-xs text-gray-500 w-12">{role}</span>
                         <select
                          value={getClosestTierValue(editingPlayer.skills[role])}
                          onChange={(e) => updateSkill(role, e.target.value, true)}
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  {/* Preferred Heroes */}
                  {player.preferredHeroes && player.preferredHeroes.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-gray-100">
                      {player.preferredHeroes.map(hero => (
                        <span key={hero} className="text-[10px] bg-indigo-50 text-indigo-700 px-1.5 py-0.5 rounded border border-indigo-100">
                          {hero}
                        </span>
                      ))}
                    </div>
